...
/*   Updated: 2019/11/25 14:45:11 by aguenel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	if (BUFFER_SIZE == 0 || !line || (!s[fd] && !(s[fd] = ft_strnew(1))) ||\
		(!(*line = ft_strnew(0))) || (fd < 0))
		return (-1);
	res = ft_fill_s(&s[fd], line, fd);
	if (s[fd][ft_strlen(*line)] == '\n')
	{
